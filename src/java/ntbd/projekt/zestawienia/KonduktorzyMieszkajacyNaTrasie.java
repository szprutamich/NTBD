...
public class KonduktorzyMieszkajacyNaTrasie {
    ...
    private static void wypiszKonduktorow(PersistenceManager pm) {
        ...
        if (wynik.size() == 0)
            System.out
                    .println("Nie znaleziono zadnego konduktora spelniajacego dane wymogi.");
        else {
            System.out
                    .println("Konduktor mieszkajacy w miescie, z ktorego lub do ktorego kursuje polaczenie:");
            int nr = 1;
            for (Konduktor k : wynik) {
                System.out.println("\t" + nr + " " + k.getImie() + " "
                        + k.getNazwisko() + " " + k.getPesel());
                nr++;
            }
        }
    }
    ...
    private static PersistenceManager getPM() throws IOException {
        ...
        InputStream is = KonduktorzyMieszkajacyNaTrasie.class.getClassLoader()
